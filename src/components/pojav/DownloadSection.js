...
const PojavDownloadSection = ({ onAchievement = () => {} }) => {
  ...
  const currentVersionData = getPojavVersionData(activeTab);
  ...
  return (
    <section id="download" className="py-12 px-6 max-w-5xl mx-auto">
      <h2 className="text-3xl font-bold text-center text-red-900 dark:text-red-400 mb-6">Descargar para Pojav Launcher</h2>
      ...
      <div className="mb-8 rounded-xl overflow-hidden shadow-lg">
        <img 
          ...
          alt={`Minecraft ${activeTab} update - ${currentVersionData.title}`}
        ...
        <div className="bg-gradient-to-r from-red-400 to-red-400 dark:from-red-800 dark:to-red-900 p-8">
          <h3 className="text-2xl font-bold text-white mb-3">Minecraft {activeTab} - Pojav Edition</h3>
          <p className="text-red-100 text-xl">
            {currentVersionData.title}
          </p>
          <p className="text-red-200 text-sm mt-2">
            📱 Optimizado especialmente para Pojav Launcher
          </p>
        </div>
      </div>
        ...
        {currentVersionData.releases.map((item) => (
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200">
                  v{item.version} - Minecraft {activeTab} (Pojav)
                </h3>
                {item.isBeta && (
            ...
            <a
              ...
              download
            >
              Descargar v{item.version} (Pojav)
            </a>
          </div>
        ))}
      </div>
...
export default PojavDownloadSection;